...
### Stage 1 — Hard Filter: Golden Triangle

The first stage enforces DSW's primary geographic mandate. A startup passes if its registered postcode falls within the **Golden Triangle** — the cluster of deep-tech innovation centred on London, Oxford, and Cambridge, extended to include Guildford, Reading, and Hertford. Postcode matching covers all relevant district prefixes (EC, WC, W1, SW, SE, OX, CB, GU, RG, SG13/14, and others).

Companies outside this geography are **not automatically eliminated**: any startup with at least one female co-founder is granted an override and proceeds to scoring. This reflects DSW's commitment to portfolio diversity and recognition that founding talent is not geographically bounded.
